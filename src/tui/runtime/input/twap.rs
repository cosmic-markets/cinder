...
/// 2 = total size, 3 = total time hours, 4 = total time minutes. ↑↓ moves
/// between fields, Tab toggles side, ←→ cycles the market when row 0 is
/// focused, digits/`.` edit numeric fields, Enter validates & spawns a bot,
/// Esc discards.
pub(in crate::tui::runtime) fn handle_editing_twap_key(
    ...
    match code {
        ...
        KeyCode::Char(c) if c.is_ascii_digit() || c == '.' => {
            // Hours and minutes are integer-only; reject `.` on those rows.
...
fn build_bot_from_draft(
    ...
    // One slice per minute (matches Binance's default cadence). Total time
    // entirely determines the schedule.
    ...
    let duration_secs: u64 = (total_minutes as u64).saturating_mul(60).max(1);
    Ok(TwapBot::new(
